...
for item, i in enumerate(path_list):
    ...
    for num in p4:
        ...
        pca.fit(X)
    arr_x.append(pca.transform(X).tolist()[0][0])
    arr_y.append(pca.transform(X).tolist()[0][1])
    ...
    def show_picture(data, center_point, cls, k):
        num, dim = data.shape
        color = ['r', 'g', 'b', 'c', 'y', 'm', 'k', 'peru', 'slategray', 'springgreen']
        if dim == 2:
            for i in range(num):
                mark = int(cls[i])
                plt.plot(data[i, 0], data[i, 1], color=color[mark], marker='o', markersize=3)
                # plt.plot(data[i, 0], data[i, 1])
            # 下面把中心点单独标记出来：
            for i in range(k):
                plt.plot(center_point[i, 0], center_point[i, 1], color=color[i], marker='x')
