...
for item, i in enumerate(path_list):
    ...
    for num_car_attr, num_objs, num_lines, num_points \
        ...
        if (num_car_attr[2] == 0 or num_car_attr[3] == 0 or num_car_attr[0] == 0):
            ...
            temp.append(1 / (num_car_attr[1] * num_car_attr[2] * num_car_attr[3]) % 100 * temp[-1])
            # complex.append(abs(int(num_track_id) * (p_car_attr[0][0])))%100
            # complex.append(abs(int(num_track_id) * (p_car_attr[0][0]))%100)
        list_conf_perframe.append(copy.deepcopy(temp_conf))
        ...
        # complex_max.append(numpy.max(complex_perframe))
        # complex_min.append(numpy.min(complex_perframe))
        # complex_avg.append(numpy.average(complex_perframe))
    ...
    x = list(range(1, len(num_car_attr) + 1, 1))
...
# print(len(x))
# print(len(complex))
# plt.plot(x,complex_avg)
# plt.show()
# complex_perframe.clear()
# list_conf_perframe.clear()
x = list(range(1, len(path_list) + 1, 1))
complex_max.append(numpy.max(complex))
complex_min.append(numpy.min(complex))
complex_avg.append(complex)
plt.plot(x, complex_avg)
plt.show()
